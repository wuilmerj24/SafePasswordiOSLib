...
#if canImport(safe_passwordFFI)
import safe_passwordFFI
#endif
...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_safe_password_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_safe_password_rustbuffer_free(self, $0) }
    }
}
...
private func makeRustCall<T, E: Swift.Error>(
    ...
    uniffiEnsureSafePasswordInitialized()
...
public func generatePassword(length: UInt64, uppercase: Bool, numbers: Bool, symbols: Bool) -> String  {
    ...
    uniffi_safe_password_fn_func_generate_password(
        ...
        FfiConverterBool.lower(uppercase),
        FfiConverterBool.lower(numbers),
        FfiConverterBool.lower(symbols),$0
    )
})
}
...
private let initializationResult: InitializationResult = {
    ...
    let scaffolding_contract_version = ffi_safe_password_uniffi_contract_version()
    if bindings_contract_version != scaffolding_contract_version {
        return InitializationResult.contractVersionMismatch
    }
    if (uniffi_safe_password_checksum_func_check_strength() != 28484) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_safe_password_checksum_func_generate_password() != 9469) {
        return InitializationResult.apiChecksumMismatch
    }
...
public func uniffiEnsureSafePasswordInitialized() {
